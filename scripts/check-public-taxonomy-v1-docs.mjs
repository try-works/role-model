...
const roots = ["docs/public", "docs/protocol"];
...
const files = (
  await Promise.all(roots.map((root) => listMarkdownFiles(path.resolve(root))))
).flat();
...
for (const token of requiredTokens) {
  if (!combined.includes(token)) {
    failures.push(`public docs taxonomy V1 content is missing required token ${token}`);
  }
}
...
for (const [key, value] of Object.entries(expectedCounts)) {
  ...
  if (!combined.includes(`${key}: ${value}`) && !combined.includes(`"${key}": ${value}`)) {
    failures.push(`public docs taxonomy V1 content is missing canonical count ${key}: ${value}`);
  }
}
...
for (const id of requiredIds) {
  if (!combined.includes(`\`${id}\``)) {
    failures.push(`public docs taxonomy V1 content is missing canonical identifier \`${id}\``);
  }
}
...
for (const taskId of requiredGuidanceTaskIds) {
  ...
  for (const [field, value] of [
    ...
    if (typeof value !== "string" || !combined.includes(value)) {
      failures.push(`public docs taxonomy V1 content is missing ${field} for ${taskId}`);
    }
  }
}
...
console.log(
  `public taxonomy V1 docs/hash parity check passed for ${files.length} Markdown files and ${requiredIds.length} canonical identifiers`,
);
