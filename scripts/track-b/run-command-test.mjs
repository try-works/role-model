...
const commandId = (fromEnv && /^TB(?:0[0-9]|1[01])-CMD-\d{2}$/.test(fromEnv) ? fromEnv : argvCommand) ?? null;
if (!commandId) {
  throw new Error("public track-b runner requires an exact TBxx-CMD-xx argument or ROLE_MODEL_TRACK_B_COMMAND_ID");
}
const forwarded = [commandId, ...args.filter((arg) => arg !== argvCommand && arg !== commandId)];
const result = spawnSync(process.execPath, [runner, ...forwarded], { cwd: internalRoot, stdio: "inherit", env: process.env });
