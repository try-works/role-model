...
- role hint `coder`
- task type `coder.edit`
- required capabilities `code.read` and `code.write`
...
| --- | --- |
| `cli.local.coder` | `code.read`, `code.write`, `reasoning.multi_step`, `tools.function_calling` |
...
2. `acp.remote.general` is excluded with `CAPABILITY_MISSING` because it does not satisfy `code.read` and `code.write`.
