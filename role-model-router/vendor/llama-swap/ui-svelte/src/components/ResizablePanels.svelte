...
onMount(() => {
  ...
  if (saved) {
    ...
    if (!Number.isNaN(parsed) && parsed >= minSize && parsed <= 100 - minSize) {
      leftSize = parsed;
    }
  }
});
...
const containerClass = $derived(direction === "horizontal" ? "flex-row" : "flex-col");

const handleClass = $derived(
  direction === "horizontal" ? "w-2 h-full cursor-col-resize" : "w-full h-2 cursor-row-resize",
);
