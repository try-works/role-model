...
function requireHistogramData(
  values: number[],
  options?: Parameters<typeof calculateHistogramData>[1],
) {
  const result = calculateHistogramData(values, options);
  expect(result).not.toBeNull();

  if (!result) {
    throw new Error("expected histogram data");
  }
...
describe("calculateHistogramData", () => {
  describe("edge cases", () => {
    ...
    it("handles single value", () => {
      const result = requireHistogramData([42]);
    ...
    it("handles all identical values", () => {
      const result = requireHistogramData([10, 10, 10, 10, 10]);
    ...
    it("handles two distinct values", () => {
      const result = requireHistogramData([10, 20]);
      expect(result.min).toBe(10);
  ...
  describe("bin distribution", () => {
    it("bins sum to total number of values", () => {
      ...
      const result = requireHistogramData(values);
      const binSum = result.bins.reduce((s, b) => s + b, 0);
    ...
    it("places values in correct bins", () => {
      ...
      const result = requireHistogramData(values, { minBins: 3, maxBins: 3 });
  ...
  describe("bin count adaptation", () => {
    ...
    it("caps bins at maximum", () => {
      ...
      const result = requireHistogramData(values, { minBins: 5, maxBins: 10 });
