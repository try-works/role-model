...
type ProxyManager struct {
	...
	roleModelBridge *RoleModelBridge
}
...
func New(proxyConfig config.Config) *ProxyManager {
	...
	pm := &ProxyManager{
		...
		roleModelBridge: newRoleModelBridgeFromEnv(proxyLogger),
	}
...
func (pm *ProxyManager) listModelsHandler(c *gin.Context) {
	if pm.roleModelBridge != nil && pm.roleModelBridge.Enabled() {
		if err := pm.roleModelBridge.ProxyModels(c.Writer, c.Request); err != nil {
			pm.sendErrorResponse(c, http.StatusBadGateway, fmt.Sprintf("error proxying model list: %s", err.Error()))
		}
		return
	}
...
func (pm *ProxyManager) mkProxyJSONHandler(cf captureFields) func(*gin.Context) {
	return func(c *gin.Context) {
		...
		if pm.roleModelBridge != nil && pm.roleModelBridge.Enabled() {
			pm.proxyLogger.Debugf("ProxyManager using role-model bridge for model: %s", requestedModel)
			modelID = requestedModel
			nextHandler = pm.roleModelBridge.ProxyRequest
		} else if found {
