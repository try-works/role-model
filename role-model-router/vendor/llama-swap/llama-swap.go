...
func main() {
	...
	var roleModelBridgeProc *roleModelBridgeProcess
	if bridgeOptions := resolveRoleModelBridgeProcessOptions(); bridgeOptions != nil {
		if err := os.Setenv("ROLE_MODEL_BRIDGE_BASE_URL", bridgeOptions.BaseURL()); err != nil {
			mainLogger.Errorf("Unable to set ROLE_MODEL_BRIDGE_BASE_URL: %v", err)
			os.Exit(1)
		}
		mainLogger.Infof("Starting managed role-model bridge on %s", bridgeOptions.BaseURL())
		proc, err := startRoleModelBridgeProcess(mainLogger, bridgeOptions)
		if err != nil {
			mainLogger.Errorf("Unable to start managed role-model bridge: %v", err)
			os.Exit(1)
		}
		roleModelBridgeProc = proc
	}
	...
	go func() {
		for {
			...
			case syscall.SIGINT, syscall.SIGTERM:
				...
				if roleModelBridgeProc != nil {
					if err := roleModelBridgeProc.Shutdown(); err != nil {
						mainLogger.Warnf("Role-model bridge shutdown failed: %v", err)
					}
				}
