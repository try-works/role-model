...
export default function RequestsRoute() {
  ...
  const distinctEndpoints = new Set(requests.map((request) => request.endpointId).filter(Boolean)).size;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <FactCard label="Requests" value={requests.length} detail="Structured requests currently available in the telemetry ledger." emphasis />
        <FactCard label="Endpoint-linked" value={requestsWithEndpoint} detail="Requests already tied to a concrete endpoint profile." />
        <FactCard label="Distinct endpoints" value={distinctEndpoints} detail={`${localRequests} local / ${remoteRequests} remote requests represented in the current ledger.`} />
      </div>

      <div className="grid grid-cols-12 gap-4">
        <SectionCard className="col-span-12 xl:col-span-8" title="Recent telemetry requests" description="Open any item to inspect the structured request artifact, endpoint profile, and captures.">
          {ledgerRows.length === 0 ? (
              ...
              {ledgerRows.map((request) => (
                <div key={request.requestId} className={`${listRowClassName} md:items-center`}>
                  ...
                  <div className="text-right">
                    <p className="text-xs uppercase tracking-[0.24em] text-[var(--rm-muted)]">{request.createdAtLabel}</p>
                    <Link className="mt-2 inline-block text-sm font-medium text-[var(--rm-accent)]" to={`/app/observe/requests/${request.requestId}`}>
                      Inspect
                    </Link>
                  </div>
                </div>
        ...
        <SectionCard className="col-span-12 xl:col-span-4" title="Inspection path" description="Keep the request ledger narrow in scope so the detail page can own deep debugging.">
          <div className="space-y-3">
            ...
            <div className={`${mutedPanelClassName} p-4 text-sm text-[var(--rm-secondary)]`}>
              ...
              <p className="mt-2">Use Activity for broader raw host activity and Logs for proxy/upstream output when the telemetry ledger is still too narrow.</p>
            </div>
          </div>
        </SectionCard>
      </div>
