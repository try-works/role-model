...
export default function IntegrationsDownstreamRoute() {
  ...
  useEffect(() => {
    void fetchDownstreamOpenAIProviderConfig()
      .then(setProvider)
      .catch((value: unknown) => setError(value instanceof Error ? value.message : "Could not load downstream compatibility details."));
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard title="Connection contract" description="Base URL, auth expectations, and downstream model discovery in one reference block.">
      ...
      <SectionCard title="Consumer setup" description="The runtime remains downstream-compatible, including tool-calling consumer expectations.">
        <div className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
          <div className={`${mutedPanelClassName} p-4`}>
            ...
            <p className="mt-4 text-sm text-[var(--rm-secondary)]">{provider.authentication.note}</p>
          </div>
            ...
            <div className={`${mutedPanelClassName} p-4`}>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {guide.availableModels.map((modelId) => (
                  <StatusPill key={modelId} tone={modelId === provider.setup.recommendedModel ? "accent" : "neutral"}>
                    {modelId}
                  </StatusPill>
                ))}
              </div>
      ...
      <SectionCard title="Compatibility posture" description="Compatibility notes stay with the downstream contract instead of on a duplicate reference page.">
        <div className="grid gap-3 md:grid-cols-3">
          <div className={`${mutedPanelClassName} p-4 text-sm text-[var(--rm-secondary)]`}>
            ...
            <p className="mt-2">Downstream consumers should treat this runtime as an OpenAI-compatible provider surfaced through the model listing and chat-completions contract shown above.</p>
          </div>
          <div className={`${mutedPanelClassName} p-4 text-sm text-[var(--rm-secondary)]`}>
            <p className="font-medium text-[var(--rm-fg)]">Tool calling</p>
            <p className="mt-2">`tool_calls` compatibility follows the selected routed model and remains inspectable in request detail rather than hidden behind a separate compatibility matrix.</p>
          </div>
          <div className={`${mutedPanelClassName} p-4 text-sm text-[var(--rm-secondary)]`}>
            <p className="font-medium text-[var(--rm-fg)]">MCP boundary</p>
            <p className="mt-2">MCP and runtime-owned tool execution stay backend-governed; downstream clients consume the compatible response surface rather than negotiating connector state directly.</p>
          </div>
        </div>
      </SectionCard>
    </div>
