...
7. **Analytics routes and evidence routes stay distinct.** `/app` and charted Observe pages lead with analytics bands; raw-host and request-detail pages remain evidence-first. Canonical telemetry pages (`/app/observe/requests`, `/app/observe/requests/:requestId`) must not embed a redundant “Adjacent raw-host tools” panel; Activity and Logs remain first-class Observe destinations in the shell nav.
