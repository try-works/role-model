...
/**
 * Run 100 R2 / RC-2 — an arm's comparable output comes from its durable branch capture.
 *
 * Live stage (`:3457`, released candidate `d19dcff3`): three arms of `req-7c747650` closed with
 * terminal assistant messages in the artifact store, yet the capture deferred with
 * `durable replay evaluation is missing counterfactual output evidence` (23:04:00Z) and, in a second
 * class, `durable replay evaluation is missing branch capture for <endpoint>` (HTTP 409, 23:09:55Z and
 * 23:30:33Z). The same window logged `Track B route capture failed … route capture skipped: 4000446
 * bytes exceeds the 524288-byte deferred capture budget`, so an arm's branch capture can be absent by
 * policy rather than by failure.
 *
 * The requirement is explicit: a tool-call-shaped answer (empty assistant text, tool calls present) is
 * comparable evidence, and an arm whose capture genuinely cannot be produced is a **named per-arm
 * exclusion** that must not cost the other arms' evidence.
 */
...
describe("run 100 R2 durable arm output", () => {
  test("an arm whose branch capture is missing is excluded by name and the capture still evaluates", async () => {
    ...
    const exclusions = (result.counterfactualExclusions ?? []) as Record<string, unknown>[];
    expect(exclusions.map((entry) => entry.endpointId)).toEqual(["endpoint:b"]);
    expect(String(exclusions[0]?.reason ?? "")).toMatch(/branch capture/u);

    // The surviving arm still reaches evaluation as the comparison's counterfactual.
  ...
  test("a tool-call-only arm is comparable evidence, not a missing-output refusal", async () => {
    const scenario = buildScenario(["b"], undefined, {
      armOutput: { b: "tool_calls" },
    });

    const result = (await scenario.invoke()) as Record<string, unknown>;
    expect(result).toMatchObject({ outcome: "candidate" });
    expect(result.counterfactualExclusions ?? []).toEqual([]);
    ...
    expect(String(counterfactuals[0]?.evaluationActual ?? "")).toMatch(/bash/u);
  });
});
