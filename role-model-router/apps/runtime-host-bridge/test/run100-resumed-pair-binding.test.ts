...
/**
 * Run 100 R3 / RC-1 — a resumed completion must finalise the pair the durable job declared.
 *
 * Live stage (`:3457`, released candidate `d19dcff3`): 146 of 711 durable evaluation jobs are `failed`
 * with `evaluation_job_stranded_without_finalized_comparison`, every one of their 477 trials is
 * `scored`, and none has a comparison group - while all 547 completed jobs hold exactly two trials.
 * The primary pair finalises when the *declared* two sides are submitted (see
 * `tests/track-b/run100-multi-arm-comparison.test.mjs`), so the live loss comes from a completion that
 * submits a pair derived from the current run's arm order instead of the pair the job declared - which
 * is exactly what a resume does when the recorded counterfactual order differs from the job's.
 *
 * These tests pin that behaviour: with a stored job whose comparability declares the pair, the finalise
 * must use the job's pair; without a stored job, the caller's pair stays authoritative.
 */
...
describe("run 100 R3 resumed pair binding", () => {
  test("a resumed completion finalises the pair the durable job declared, not the resumed arm order", async () => {
    // The durable job declared source + arm b; the resume's arm order starts with arm c.
    const scenario = buildScenario(["c", "b"], undefined, {
      storedJob: durableMultiArmJob({
        declaredCounterfactual: "endpoint:b",
        allCandidates: ["endpoint:1", "endpoint:b", "endpoint:c"],
      }),
    });
    ...
    expect(primaryFinalizer).toBeDefined();
    const submitted = (primaryFinalizer?.trialIds as string[] | undefined) ?? [];
    expect(submitted.slice().sort()).toEqual(
      [trialIdFor("endpoint:1"), trialIdFor("endpoint:b")].sort(),
    );
  });

  test("without a durable job the caller's primary pair stays authoritative", async () => {
    const scenario = buildScenario(["c", "b"]);
    const result = await scenario.invoke();
    expect(result).toMatchObject({ outcome: "candidate" });
    ...
    expect(
      (primaryFinalizer?.comparability as Record<string, unknown> | undefined)
        ?.counterfactualCandidateRef,
    ).toBe("endpoint:c");
  });
});
