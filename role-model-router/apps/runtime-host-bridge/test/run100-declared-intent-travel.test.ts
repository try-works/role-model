...
function buildScenario(
  ...
  options: {
    ...
    /** Run 100 R5: the taxonomy identity the source request declared and was classified under. */
  ...
  const sourceCapture = capture("1", {
    ...(options.sourceClassification
      ? {
          taskTypeId: options.sourceClassification.taskTypeId,
          roleId: options.sourceClassification.roleId,
          taxonomyVersion: options.sourceClassification.taxonomyVersion,
          classification: {
            taskTypeId: options.sourceClassification.taskTypeId,
            roleId: options.sourceClassification.roleId,
            taxonomyVersion: options.sourceClassification.taxonomyVersion,
            toolClassIds: [...(options.sourceClassification.toolClassIds ?? [])],
          },
        }
      : {}),
  });
...
/**
 * Run 100 R5 - the declared intent's taxonomy identity travels with the capture into the comparison.
 *
 * The declared `role_model.intent` (task type, role hint, tool classes) and the taxonomy version the
 * request was classified against are stamped on the observation
 * (`apps/runtime-host-bridge/src/index.ts`: `buildRequestClassification({ taskTypeId, roleId, toolClasses })`
 * plus `taxonomyVersion` from the taxonomy manifest), the capture carries them, and the supervised-replay
 * completer passes them into the pipeline (`cli.ts`: `taskTypeId`, `taxonomyVersion`, `roleId`), which
 * writes them into the durable job's comparability. This test proves that last leg - capture -> replay ->
 * comparison - for a declared `coder.edit` request carrying a role hint and tool classes, so the learner's
 * family-scoped floor and the pack scope are keyed by the taxonomy the request declared.
 */
describe("run 100 R5 declared-intent travel", () => {
  test("the capture's declared taxonomy identity reaches the comparison's comparability", async () => {
