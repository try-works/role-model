---
name: role-model
description: Use when Pi should route model requests through a locally running Role-Model runtime, inspect Role-Model aliases, diagnose Role-Model provider setup, or inspect runtime-owned request explanations.
---
...
For runtime request inspection, use `/role-model requests` to list the latest runtime-owned requests and `/role-model explain latest` to show the runtime's own explanation for the latest request. Those commands must surface runtime-owned request, routing, benchmark, and telemetry evidence when available rather than synthesizing new reason codes in Pi.
