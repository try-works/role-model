...
const taxonomySchemaDir = path.join(repoRoot, "schemas", "role-model", "taxonomy");
...
async function loadTaxonomySchemas(): Promise<
  Array<{ fileName: string; schema: CanonicalJsonSchema }>
> {
  const names = (await readdir(taxonomySchemaDir))
    .filter((name) => name.endsWith(".schema.json"))
    .sort();
  return Promise.all(
    names.map(async (name) => {
      const fileName = path.posix.join("schemas", "role-model", "taxonomy", name);
      const schema = JSON.parse(
        await readFile(path.join(taxonomySchemaDir, name), "utf8"),
      ) as JsonSchema;

      return {
        fileName,
        schema: assertStableExternalSchemaId(fileName, schema),
      };
    }),
  );
}

function assertStableExternalSchemaId(fileName: string, schema: JsonSchema): CanonicalJsonSchema {
  const schemaId = schema.$id;
...
export async function generateProtocolTypes(outputPath = protocolTypesOutput): Promise<void> {
  const schemas = await loadProtocolSchemas();
  const blocks: string[] = [
