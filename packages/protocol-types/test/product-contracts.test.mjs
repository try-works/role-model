...
const contract = JSON.parse(
  await readFile(new URL("../generated/product-contracts.json", import.meta.url), "utf8"),
);

test("TB00-TEST-INTEGRATION keeps non-production contribution and sensitive permissions fail-closed", () => {
  assert.equal(
    contract.defaults.production.aggregateContribution,
    "advanced_aggregate_only_after_disclosure",
  );
