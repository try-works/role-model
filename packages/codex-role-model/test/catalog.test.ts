...
describe("Codex ModelsResponse catalog generation", () => {
  test("builds non-empty catalog from golden fixture and aliases", () => {
    ...
    const { catalog } = buildModelsCatalog(createDiscoveryResult(), { integrationMode: "signed-in" });
    ...
    const roleModel = models.filter((m) => !m.slug.startsWith("gpt-") && m.slug !== "codex-auto-review");
