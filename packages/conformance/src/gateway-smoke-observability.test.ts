...
describe("gateway smoke observability conformance", () => {
  test("emits trace and usage artifacts with run01 linkage ids and canonical trace shapes", async () => {
  ...
  test("emits observed performance with the run01 sample window and sources shape", async () => {
    await rm(smokeOutputDir, { recursive: true, force: true });
    await runSmokeApp();
