...
- runtime routing modes such as `baseline`, `controller`, `difficulty`, and `hybrid`
...
- the current `baseline-v2` scoring version
...
The packaged runtime also introduces implementation-level routing modes that sit above the pure policy
strategy layer. Those modes are useful product behavior, but they are not themselves part of the canonical
protocol strategy enum.
