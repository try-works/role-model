...
description: A high-level tour of the protocol objects that let role-model describe requests, endpoints, policy, and explainable routing outcomes.
---
...
| `RoleDefinition` | describes a role such as `support` or `coder` |
...
The deeper protocol pages on this site start at [Endpoint identity](/protocol/endpoint-identity) and run
through [Trace and usage artifacts](/protocol/trace-and-usage-artifacts).
...
- capabilities such as `code.write`, `reasoning.multi_step`, and `tools.function_calling`
