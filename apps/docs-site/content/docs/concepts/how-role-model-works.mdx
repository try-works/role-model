...
- required capability `coder.edit`
...
| --- | --- |
| `cli.local.coder` | `code.write`, `reasoning.multi_step`, `tools.function_calling` |
...
2. `acp.remote.general` is excluded with `CAPABILITY_MISSING` because it does not satisfy `code.write`.
