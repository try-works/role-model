...
- `strategy` describes scoring intent such as quality vs latency vs cost
- `compute_preference` describes locality posture such as local vs remote

They are related, but they are not the same control.

`hybrid` in `compute_preference` should be understood as protocol surface that keeps both pools expressible.
The current baseline router most directly applies locality preference when the value is `local` or `remote`.
