...
- Private TB00 product pin is `726df64d241ba03eb34eed35f1785a3963ba0057`; public product pin remains `b03d82a2fe8adc317c9fdaecad838beac3ed74a8`.
...
- SEA used for Phase 5: sha256 `caa7c9e7a8a0c3ef57a0aaf801d97cd1021817db1443d4d8d7e5e4f97806b424` (`run85-dev`).
- Server change for run 85 was `not-required`; `publicChange: required`.
...
- Prefer run-85 evidence under `.recursive/run/85-kw-gated-router-prompt-inject/evidence/` for inject unlock, host join/auto-arm, pin/freeze, and Phase 5 rebuild/SEA-inject/probe/cloud/`pi` proofs (`binder.json`, Phase 4/5 logs, rebuild receipt).
...
- KW probe: `scripts/track-b/run81-kw-activation-probe.mjs` (inject matrix); SEA inject hop helper: `scripts/track-b/run85-sea-inject-hop.mts`; TB10: `tests/track-b/tb10.test.mjs`; assemble: `scripts/track-b/assemble-run00-live-e2e.mjs`.
...
- Package SEA with `ROLE_MODEL_TRACK_B_DISTRIBUTION_ROOT` set; wire private KW join factory on Track B manifest before claiming SEA inject unlock.
